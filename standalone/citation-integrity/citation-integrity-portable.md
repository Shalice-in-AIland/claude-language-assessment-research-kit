...
1. **DOI resolve & match.** For every entry with a DOI: check what it actually resolves to (`https://doi.org/<doi>`). Two failure modes, both findings: it resolves to **nothing** (dead or fabricated), or to a **different work** than the entry describes (compare title, first author, year — quote what you found). **Absence from one database is never proof of death**: dataset, software, and repository DOIs (Zenodo, OSF, figshare) are missing from journal indexes by design — check `doi.org` itself and a second source before calling any DOI dead, and report "couldn't check" (network/tool failure) separately from "not found anywhere."
