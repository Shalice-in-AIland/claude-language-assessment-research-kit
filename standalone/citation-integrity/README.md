...
| [`self-check-sample.md`](self-check-sample.md) + [`self-check-answer-key.md`](self-check-answer-key.md) + [`self-check-protocol.md`](self-check-protocol.md) | **Anyone — see the check work, or compare tools** | A deliberately flawed passage + reference list to paste with the portable file, an answer key, and a step-by-step protocol (identical prompt, four standardised follow-ups, a scoring sheet) so runs are comparable across AI tools. Nothing in the sample may be cited. |
