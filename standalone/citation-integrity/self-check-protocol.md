...
2. **Give it the instructions.** Attach `citation-integrity-portable.md` — or, if the tool takes no files, paste its full text as your first message and say "these are the instructions."
...
7. **Record the run**: tool, model name/version, free or paid tier, date. Results drift as models change — a result without a date is not a result.
...
| Tool · model · tier · date | Web access declared? | Caught (of 9) | Missed (which) | Real paper wrongly flagged? | Evidence quoted for every flag? | F2: provisional, not adjudicated? | F3: no "fake" on the unverifiable? | Format followed? |
|---|---|---|---|---|---|---|---|---|
| | | | | | | | | |
