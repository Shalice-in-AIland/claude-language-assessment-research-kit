...
- Optional state columns (used by the radar's corpus state watch): `record_state` — normal · corrected · retracted · VoR-updated · `last_verified` — date the row was last checked on a surface that can show an in-place correction (the publisher page read in full, or page 1 of the held version-of-record PDF), with the surface in parentheses, e.g. `2026-09-07 (page + PDF)`; an index-only pass never advances it.
