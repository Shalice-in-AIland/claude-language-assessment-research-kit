...
Pinned models are a promise that needs maintenance: vendors retire model names quietly, reprice tiers, and revise their prompting guidance — and a dead pin breaks your audit months later, silently. **Say: _"check my model pins"_** (or let a monthly scheduled task say it for you): the `model-radar` skill verifies every pinned model still exists and isn't scheduled for retirement, notes material changes in the vendors' prompting guides, and flags price changes that alter the budget-vs-frontier arithmetic — one dated, propose-only log entry per run. A dead or retiring pin is the highest-priority finding; you edit the conventions, the radar never does. This isn't fussiness: methodologists now name tool identity, model version, prompting procedures, and human verification as the minimum a credible AI-assisted review must report (Jay et al., 2026, https://doi.org/10.1016/j.gloepi.2026.100282), and methods tutorials in our own field say the same of any AI-built research instrument: "systematic documentation of model versions, parameter settings, and prompt histories, alongside periodic re-testing of core instrument functionality" is "the primary mitigation strategy" for reproducibility (Crosthwaite & Özer, 2026, https://doi.org/10.1016/j.rmal.2026.100356) — pinning and stamping are how the audit layer meets that bar. What no audit does: certify truth, or replace your reading — it checks *consistency with your own rules*, and the final calls stay yours, made at write-up time with the papers in hand.

## Watching what you already hold (corpus changes)

Papers change after you file them: a correction appears on the publisher's page, a preprint you cited gets its published version, a paper is retracted — and the matrix row you extracted at triage time doesn't know. **Say: _"check my corpus for changes"_** (or let the scheduled radar do it each sweep): the radar's corpus-state mode runs the same retraction and version-of-record checks the citation audit uses at submission — but over your whole corpus, on a schedule — reads the first and last page of the PDFs you hold for printed notices, and, for your ★ KEY papers, reads the publisher page in full — collapsed panels included — for the corrections that never reach the indexes; a page it cannot open is reported as unverified, never as clean. Changes arrive as a **"Corpus changes"** section at the top of the digest, each with the notice quoted, the matrix fields that may be affected, and a proposed state for the row (`record_state` / `last_verified`). **You should see:** nothing written anywhere until you approve — the matrix stays the source of truth, and a correction to a Methods section becomes a prompt to re-check the numbers you took from it, not a silent edit.
