...
> **CLARK — the whole research workflow: operated by Claude, checked at every step, decided by you.** It doesn't read your papers for you and won't write your argument. It runs the machinery *around* the scholarship: renders your citations perfectly, audits every reference against live records (dead DOIs, mis-attached works, retractions, superseded preprints), watches the field for new papers, recomputes the statistics in your manuscript, and reads your draft through your target journal's *own* reviewer criteria — with a second, different-vendor AI on call to re-judge any of it. What stays yours is everything that makes it research: the reading, the relevance calls, the interpretation, and the claims you put your name to. **Claude is the operator and the checker — never the author.**

| Skill | What it does | Guide | Standalone |
|---|---|---|---|
| [`zotero-citations`](skills/zotero-citations/SKILL.md) | Automatic citations & reference lists — Zotero → Better BibTeX → Pandoc → Word, with guided setup for first-time users | [docs/01](docs/01-zotero-setup.md)–[03](docs/03-troubleshooting.md) | — |
| [`citation-integrity`](skills/citation-integrity/SKILL.md) | Pre-submission reference audit — every DOI resolves **and matches** your entry; retraction check (Crossref/Retraction Watch); a 16-rule integrity charter aligned to COPE/ICMJE | [docs/04](docs/04-citation-integrity.md) | [portable pack](standalone/citation-integrity/README.md) |
| [`pre-submission-review`](skills/pre-submission-review/SKILL.md) | Evidence-anchored evaluation of YOUR OWN manuscript before submission — verification ledger, fatal/major/minor triage, a prioritized revision plan + a 0–100 quality estimate, and a compliance check against the target journal's current author instructions; original research and research syntheses | [docs/08](docs/08-pre-submission-review.md) | — |
| [`writing-polish`](skills/writing-polish/SKILL.md) | Polish of YOUR OWN prose across five dimensions — accuracy, clarity/readability, formality, register & conventions, stance — built for multilingual and early-career academics; accuracy errors corrected directly (with a learnable change log), style as numbered proposals you accept one at a time; numbers/citations/claim strength never touched; the AI-declaration reminder is built in | [docs/09](docs/09-writing-polish.md) | — |
| [`literature-review`](skills/literature-review/SKILL.md) | Living review matrix — approval-gated triage (relevance tiers, KEY flags, keep-but-flag quality checklist) + Obsidian views generated from the matrix (citekey-named notes, controlled theme hubs, method/tool hubs) | [docs/07](docs/07-literature-review.md) | — |
| [`literature-radar`](skills/literature-radar/SKILL.md) | Standing watch for new papers — scheduled sweeps, strict dedup against your corpus, pre-screened triage-ready digests with a "needs your attention" shortlist — plus a **corpus-state watch** over the papers you already hold (corrections, retractions, versions of record); propose-only, you approve every inclusion and change | [docs/07](docs/07-literature-review.md) | — |
| [`model-radar`](skills/model-radar/SKILL.md) | Maintenance watch on the AI models your audits pin — monthly liveness check of pinned model IDs (aliases retire quietly), vendor prompting-guide deltas, price/tier changes; propose-only log | [docs/07](docs/07-literature-review.md#the-model-radar) | — |

*Every skill also works **on its own** — run just the citation check, or just a review, in any order. And a skill is a plain instruction file, not an app: the full machinery lives in Claude Code (renders, live Zotero, scheduled radars), and a skill can travel to lighter setups — paste one into a chat interface and Claude follows the same workflow within that interface's limits. The **Standalone** column marks the skills already packaged for exactly that: a self-contained edition for any AI chat, no repo needed.*

*CLARK's home field is **language assessment** — the reviewer criteria, venue guides, and examples are built from that field's own materials, not from a generic research template. But nothing in the machinery is assessment-specific: if you work anywhere in **applied linguistics** (SLA, CALL, EAP/L2 writing, corpus research…), the pipeline works as-is — your journals join the reviewer-guides file on demand. The rest of this page documents the citation skills — the toolkit's foundation.*
...
*Full history: [CHANGELOG.md](CHANGELOG.md) — every release is archived on Zenodo. Citing CLARK in a paper: [docs/00 → Reporting CLARK in your methods section](docs/00-using-clark-in-your-project.md#reporting-clark-in-your-methods-section). What CLARK learns from: [docs/10](docs/10-sources.md).*
...
| Tool | Think of it as |
|---|---|
| [Zotero](https://www.zotero.org) | your **library catalogue** — drop a PDF in, it fetches the details (guided setup: [docs/01](docs/01-zotero-setup.md)) |
| [Better BibTeX](https://retorque.re/zotero-better-bibtex/) (a Zotero plugin) | the **ID-card printer** — every paper gets a permanent citekey like `cheng2026` (installed in [docs/01](docs/01-zotero-setup.md), step 2) |
| [Pandoc](https://pandoc.org) | the **typesetter** — turns your draft + IDs into a finished Word document (Claude installs it; by-hand route: [docs/03](docs/03-troubleshooting.md)) |
...
| [Claude Code](https://claude.com/claude-code) | the **built-in operator** — walks you through setup, runs the renders, reads every warning, audits your library (project wiring & session habits: [docs/00](docs/00-using-clark-in-your-project.md)) |
...
- **Zero setup, any AI chat** *(~2 min)*: skip every step above — no repo, no Claude Code. Download just the **[standalone pack](standalone/citation-integrity/README.md)**, paste its one self-contained file into any AI chat as your first message, then your reference list, and say *"check my citations."* (Currently the citation check; more portable editions are planned — the **Standalone** column above tracks them.)
...
**Level 2 (recommended once the basics work):** give Claude a *live* connection to your Zotero library — it verifies records against Zotero itself, finds papers by *meaning* (semantic search), and spots duplicates/missing DOIs while you write. Under the hood this is an **MCP connection** (Model Context Protocol — the open standard for giving AI assistants live access to tools; we consume the open-source [zotero-mcp](https://github.com/54yyyu/zotero-mcp) server). **Claude installs this for you**: say *"Read docs/05 and connect yourself to my Zotero library"* — your only manual part is one checkbox in Zotero. Details & what it adds: [docs/05](docs/05-zotero-mcp.md). Reversible; everything stays on your computer.
...
**Take just the citation check with you:** [`standalone/citation-integrity/`](standalone/citation-integrity/README.md) packages it in two portable forms — a **self-contained edition you can paste into any AI assistant** ("check my citations", verified by web search, reported as an action-first fix list), and a **one-page practice checklist for research students and their supervisors** (no AI required; printable .docx included). Both carry the same standing rule: an AI may format and check a citation — it may never supply one.
